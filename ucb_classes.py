...
class robust_lilUCB():
    def __init__(self, δ, ϵ, β, K, theoretical_cst=False, n0=1):
        self.δ = δ
        self.ϵ = ϵ
        self.β = β
        self.λ = ((2+self.β)/self.β)**2
        ...
        self.σ = .5
        self.r = 0.5
        #self.α = np.exp(-0.5/self.σ**2)/np.sqrt(2*np.pi*self.σ**2)
        self.α = 1
    ...
    def upper_bound(self, T):
        ...
        Compute upper confidence bound for each arm
        """
        lil = np.log(np.log(T/self.δ)/self.δ)
        
        if self.theoretical_cst==True:
            cst = (1+self.ϵ)*self.σ**2/self.α**2
        else:
            cst = 1
